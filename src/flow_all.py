...
# @flow(
#     name='Loop Files to Download', 
#     task_runner=DaskTaskRunner(cluster_kwargs={"n_workers": 8, "threads_per_worker": 1})
# )
...
def main(): #url, data_dir, download_chunk, workers):# p: Parameters):
    ...
    t6_next = []
    for f in t5_diff_l.result():
        result = cloud.download_new_csvs(url=t2_url, year=t1_year, diff_set=f, data_dir=p.data_dir)
...
DeploymentSpec(
    ...
    flow_runner=SubprocessFlowRunner(
        virtualenv='/home/ben/github/NOAA-file-download/.venv'
    )
    # parameters={
